...
export default async function signInWithOtp(req: Request): Promise<Response> {
    try {
        ...
        logger.log(LOGINFO.OTP_SEND_STARTED.replace("{phoneNo}", phoneNo));  // Log info when OTP sending starts
        ...
        if (userError) {
            ...
            logger.error(LOGERROR.USER_NOT_FOUND+ userError.message);  // Log error if user is not found
        ...
        if (user) {
            ...
            if (user.lockout_time && user.lockout_time > currentTime) {
                logger.log(LOGINFO.USER_FOUND.replace("{phoneNo}", phoneNo));  // Log user found
                return ErrorResponse(HTTP_STATUS_CODE.FORBIDDEN, `${USERMODULE.ACCOUNT_DEACTIVATED} Try after ${user.lockout_time}`);
            }

            logger.log(LOGINFO.USER_NOT_LOCKED_OUT.replace("{phoneNo}", phoneNo));  // Log user is not in lockout
        }

        // Send OTP
        const { data: _data, error } = await sendOtp(phoneNo);
        if (error) {
            logger.error(LOGERROR.OTP_SEND_ERROR+ error);  // Log error if OTP send fails
            return ErrorResponse(HTTP_STATUS_CODE.INTERNAL_SERVER_ERROR, `${error}`);
        } else {
            logger.log(LOGINFO.OTP_SENT_SUCCESS.replace("{phoneNo}", phoneNo));  // Log success message when OTP is sent
            return SuccessResponse(USERMODULE.SENT_OTP_SUCCESS, HTTP_STATUS_CODE.OK);
        }

    } catch (error) {
        logger.error(LOGERROR.INTERNAL_SERVER_ERROR+error);  // Log any internal server error
        return ErrorResponse(HTTP_STATUS_CODE.INTERNAL_SERVER_ERROR, `${error}`);
    }
}
