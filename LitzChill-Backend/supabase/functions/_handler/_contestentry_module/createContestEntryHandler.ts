...
import { validateContestId, validateUserId } from "../../_shared/_validation/ContestEntryValidation.ts";
...
export default async function handleSubmitContestEntry(req: Request, param: Record<string, string>): Promise<Response> {
  try {
    ...
    if (!userId) {
     ...
     const user_id=validateUserId(userId);
     if (user_id instanceof Response) {
      console.log("User ID validation failed, returning response:", userId); // Log if validation fails
      return user_id;
    }
    // Validate contestId
    ...
    console.log("MEme Response is: ",memeResponse);
    if (!memeResponse.ok ) {
  ...
  // Log and extract meme_id from the response
    ...
    const memeId = memeData.body.data.meme_id;
    ...
    if (errorInInsert) {
      console.log("Error in inserting contest entry:", errorInInsert);
      if(errorInInsert.code=="23505"){
        return ErrorResponse(HTTP_STATUS_CODE.BAD_REQUEST,CONETST_ENTRY_ERROR_MESSAGE.ALLREADY_REG);
      }
