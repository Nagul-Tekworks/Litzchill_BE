...
export default async function updateMemeStatus(req: Request, params: Record<string, string>) {
  try {
      ...
      console.log(`Received parameters: meme_id=${meme_id}, user_id=${user_id}`);

      // Validate `meme_id`
      if (!meme_id || !V4.isValid(meme_id)) {
          console.error("Validation failed: Invalid or missing meme_id.");
      ...
      if (!meme_status) {
          console.error("Validation failed: Missing meme_status.");
      ...
      // Notify user if the meme is rejected
          console.log("Preparing notification for meme status...");
      ...
      console.error("Unexpected error in updateMemeStatus:", error);
